...
class MixStyle(nn.Module):
    ...
    def __repr__(self):
        return f'MixStyle(p={self.p1}, alpha={self.alpha}, eps={self.eps}, mix={self.mix})'
...
class AdapterSuper(nn.Module):
    def __init__(self,
        ...
        self.adapter_conv = Conv2d_cd(reduction_dims, reduction_dims, 3, 1, 1, theta=0.7)
    ...
    def forward(self, x, identity=None):
        ...
        x_patch = x_down[:, 1:].reshape(B, 14, 14, self.super_reductuion_dim).permute(0, 3, 1, 2)

        x_patch = self.adapter_conv(x_patch)

        x_patch = x_patch.permute(0, 2, 3, 1).reshape(B, 14 * 14, self.super_reductuion_dim)
        x_cls = x_down[:, :1].reshape(B, 1, 1, self.super_reductuion_dim).permute(0, 3, 1, 2)
...
class Block(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, drop=0., attn_drop=0.,
                 drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm, visual_prompt_dim=0, LoRA_dim=1024,
                 adapter_dim=0, prefix_dim=0, drop_rate_LoRA=0, drop_rate_prompt=0, drop_rate_adapter=0):
        super().__init__()
    ...
    def set_sample_config(self, sample_LoRA_dim=None, sample_adapter_dim=None, sample_prefix_dim=None,
                          sample_last_prompt_tuning_dim=None, sample_prompt_tuning_dim=None):
...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=2, embed_dim=768, depth=24,
                num_heads=12, mlp_ratio=4., qkv_bias=True, representation_size=None, distilled=False,
                drop_rate=0., attn_drop_rate=0., drop_path_rate=0., embed_layer=PatchEmbed, norm_layer=None,
                act_layer=None, weight_init='', super_prompt_tuning_dim=0, super_LoRA_dim=1024,
                super_adapter_dim=0, super_prefix_dim=0, drop_rate_LoRA=0, drop_rate_prompt=0,
                drop_rate_adapter=0, IS_not_position_VPT=False):
        """
        Args:
    ...
    def get_sampled_params_numel(self, config):
        ...
        if self.sample_prompt_tuning_dim[0] == 0:
            ...
            if not self.IS_not_position_VPT:
                sample_prompt_tuning_param = self.visual_prompt_token[:, :self.sample_prompt_tuning_dim[0],
                                             :].numel() + self.visual_prompt_token_pos_embed.numel()
            else:
